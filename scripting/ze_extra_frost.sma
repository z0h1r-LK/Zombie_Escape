...
// Item Info.
stock const ZE_ITEM_NAME[] = "Frost Nade"
stock const ZE_ITEM_COST = 10
stock const ZE_ITEM_LIMIT = 0

// FlashBang: Ammo ID.
const ammo_flashbang = 11
...
{
	// Load Plug-In.
	register_plugin("[ZE] Extra Item: Frost Nade", ZE_VERSION, ZE_AUTHORS)

	// Cvars.
	bind_pcvar_num(register_cvar("ze_extra_frost", "1"), g_bEnabled)
...
public ze_select_item_post(id, iItem, bool:bIgnoreCost)
{
	...
	new const iAmount = rg_get_user_bpammo(id, WEAPON_FLASHBANG)
	...
	if (iAmount)
	{
		...
		send_MsgAmmoX(id, ammo_flashbang, 1)
